...
const SleepHistory = () => {
  ...
  const t = useTranslations("sleep-history");
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="flex flex-wrap md:flex-nowrap gap-4 mb-6"
      >
      ...
      {error && (
        <motion.div
          variants={variants.card}
          className="py-4 px-3 bg-red-100 text-red-800 rounded-md text-center"
        >
          {t("errorPrefix")}: {error}
        </motion.div>
      )}
          ...
          >
            ...
            >
              ...
              <p className="text-3xl font-bold text-orange-600">
                {stats.average_hours != null
                  ? `${stats.average_hours.toFixed(1)} ${t("hoursUnit")}`
                  : t("notAvailable")}
              </p>
            ...
            >
              ...
              <p className="text-3xl font-bold text-orange-600">
                {stats.max_hours != null
                  ? `${stats.max_hours.toFixed(1)} ${t("hoursUnit")}`
                  : t("notAvailable")}
              </p>
              {stats.daily_data?.length > 0 &&
                (() => {
                  ...
                  return maxDate ? (
                    <p className="text-sm text-orange-700">
                      {t("onDate")} {new Date(maxDate).toLocaleDateString()}
                    </p>
                  ) : null;
                })()}
            </motion.div>
            ...
            >
              ...
              <p className="text-3xl font-bold text-orange-600">
                {stats.min_hours != null
                  ? `${stats.min_hours.toFixed(1)} ${t("hoursUnit")}`
                  : t("notAvailable")}
              </p>
              {stats.daily_data?.length > 0 &&
                (() => {
                  ...
                  return minDate ? (
                    <p className="text-sm text-orange-700">
                      {t("onDate")} {new Date(minDate).toLocaleDateString()}
                    </p>
                  ) : null;
                })()}
            </motion.div>
              ...
              <div className="h-64 relative">
                {stats.daily_data.map((day, idx) => (
                  <motion.div
                    ...
                    style={{
                      left: `${(idx / stats.daily_data.length) * 100}%`,
                      width: `${90 / stats.daily_data.length}%`,
                      height: `${(day.hours / 12) * 100}%`,
                    }}
                    title={`${new Date(day.sleep_date).toLocaleDateString()}: ${day.hours} ${t("hoursUnit")}, ${t("qualityLabel")}: ${day.quality}/5`}
                  />
                ))}
                {[0, 4, 8, 12].map((hr) => (
                  <motion.div
                ...
                {stats.daily_data.map((day, idx) => (
                  ...
                  >
                    <div
                      ...
                      title={`${new Date(day.sleep_date).toLocaleDateString()}: ${t("qualityLabel")} ${day.quality}/5`}
                    />
                  </motion.div>
                ))}
              </div>
...
export default function Page() {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center w-full md:w-3/4 lg:w-1/2 gap-8 overflow-y-auto p-8"
    >
      <motion.h1
        variants={variants.header}
        className={`${rethinkSans.className} text-[46px] font-extrabold text-orange-600`}
      >
        {t("title")}
      </motion.h1>
      <motion.p
        variants={variants.header}
        className="text-lg font-bold text-orange-600 text-center"
      >
        {t("subtitle")}
      </motion.p>
      <motion.div
        variants={variants.formSection}
        className="flex flex-col w-full md:w-3/4 p-6 space-y-4"
      >
        <motion.div variants={variants.card} className="space-y-2">
          <h3 className="text-md font-medium text-orange-600">
            {t("selectDate")}
          </h3>
          <input
            type="date"
            ...
            max={new Date().toISOString().split("T")[0]}
            className="w-full border text-gray-700 border-orange-400 px-2 py-1 rounded-md focus:ring-2 focus:ring-orange-400"
          />
        </motion.div>
        ...
        <motion.div variants={variants.card}>
          ...
          <input
            ...
            className="w-full border text-gray-700 border-orange-400 px-2 py-1 rounded-md text-center focus:ring-2 focus:ring-orange-400"
          />
        </motion.div>
        ...
        <motion.div variants={variants.card} className="space-y-2">
          ...
          <div className="flex justify-between">
            {[1, 2, 3, 4, 5].map((r) => (
              <button
                key={r}
                onClick={() => setSleepQuality(r)}
                className={`relative w-10 h-10 rounded-full flex items-center justify-center transition-colors duration-150 focus:outline-none ${sleepQuality === r ? "" : "bg-orange-100"}`}
              >
                {sleepQuality === r && (
                  <motion.div
                    layoutId="selected-quality-bg"
                    className="absolute inset-0 bg-orange-500 rounded-full"
                    transition={{ type: "spring", stiffness: 350, damping: 30 }}
                  />
                )}
                <span
                  className={`relative transition-colors ${sleepQuality === r ? "text-white" : "text-orange-900"}`}
                >
                  {r}
                </span>
              </button>
        ...
        <motion.button
          ...
          onClick={handleSave}
          disabled={isLogging}
          className={`${isLogging ? "bg-gray-300 text-gray-500" : "bg-orange-500/25 hover:bg-orange-600/25 text-orange-600"} px-4 py-2 rounded-lg border border-orange-600 duration-50`}
        >
        ...
        {logSuccess === true && (
          <motion.div
            variants={variants.card}
            className="py-2 px-4 bg-green-100 text-green-800 rounded-md text-center"
          >
            ✓ {t("saveSuccess")}
          </motion.div>
        )}
        {logSuccess === false && (
          <motion.div
            variants={variants.card}
            className="py-2 px-4 bg-red-100 text-red-800 rounded-md text-center"
          >
            ✗ {t("saveFail")}
          </motion.div>
        )}
